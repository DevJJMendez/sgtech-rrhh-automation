<fieldset class="form-section">
    <legend>Documentos requeridos</legend>
    <div class="input-row">
        <div class="input-group input-small">
            <label for="">Fotocopia de la cédula de ciudadanía ampliada al 150%</label>
            <input type="file" name="documents[1-Fotocopia-de-la-cédula-de-ciudadanía-ampliada-al-150%]"
                accept="image/jpg, image/png, .pdf" required>
        </div>
    </div>
    <div class="input-row">
        <div class="input-group input-small">
            <label for="">Certificación bancaria</label>
            <input type="file" name="documents[2-Certificación-de-afiliación-a-EPS]" accept=".pdf" required>
        </div>
    </div>
    <div class="input-row">
        <div class="input-group input-small">
            <label for="">Antecedentes policía, procuraduría y contraloría (bajar de
                internet) (PDF)</label>
            <input type="file" name="documents[3-Antecedentes-policía-procuraduría-y-contraloría]" accept=".pdf"
                required>
        </div>
    </div>
    <div class="input-row">
        <div class="input-group input-small">
            <label for="">Fotografía para la firma de contrato</label>
            <input type="file" name="documents[5-Fotografía-para-la-firma-de-correo]" required>
        </div>
    </div>
</fieldset>
